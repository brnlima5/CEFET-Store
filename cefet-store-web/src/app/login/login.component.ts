...
export class LoginComponent {
  ...
  onSubmit(): void {
    ...
    this.authService.login(username, password).subscribe(
      (res) => {
        if(UsuarioStorageService.isAdminLoggedIn()) {
          this.router.navigateByUrl('admin/painel');
        } else if(UsuarioStorageService.isClienteLoggedIn()) {
          this.router.navigateByUrl('cliente/painel');
        }
      },
      (error) => {
        this.snackBar.open('Login ou senha incorretos', 'Fechar', { duration: 5000});
      }
    )
  }
